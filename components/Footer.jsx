			...
			<div className="max-w-7xl mx-auto">
				<div className="flex flex-col md:flex-row items-start justify-between gap-10 py-10 border-b border-slate-500/30 text-slate-500">
					<div>
						...
						<p className="max-w-[410px] mt-6 text-sm">{t("footer.welcome")}</p>
						<div className="flex items-center gap-3 mt-5">
							{socialIcons.map((item, i) => (
								<Link
									href={item.link}
									key={i}
									className="flex items-center justify-center w-10 h-10 bg-[#F5F0E8] hover:scale-105 hover:border border-orange-300 transition rounded-full"
								>
									<item.icon />
								</Link>
							))}
				...
				<p className="py-4 text-sm text-slate-500">{t("footer.copyright")}</p>
			</div>
		</footer>
	);
};
