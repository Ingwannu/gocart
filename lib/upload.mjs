...
// Public uploads are served statically from the app's own origin under
// /uploads/*, so anything a browser will render inline (svg, html, js) becomes
// same-origin script execution. Only raster images are allowed there; the
// permissive source-code policy below applies exclusively to private
// digital-download uploads, which are never served inline.
const publicUploadExtensions = new Set(["gif", "jpeg", "jpg", "png", "webp"]);
...
export function validateUploadFileMeta(file, { visibility = "public" } = {}) {
	...
	if (visibility === "public") {
		if (!publicUploadExtensions.has(extension)) {
			return {
				ok: false,
				error: "Public uploads must be an image (png, jpg, webp, gif)",
			};
		}
		if (type && !type.startsWith("image/")) {
			return { ok: false, error: "Public uploads must be an image" };
		}
		return { ok: true };
	}
