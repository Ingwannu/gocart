...
export async function putObject({
	...
	if (body instanceof Uint8Array || Buffer.isBuffer(body)) {
		await writeFile(local.storagePath, Buffer.from(body), { flag: "wx" });
		return;
	}

	// Stream to disk so multi-GB digital bundles never sit in memory. Uploads
	// hand us a web ReadableStream (file.stream()); node streams also work.
	...
	const source =
		typeof body?.getReader === "function"
			...
			: typeof body?.pipe === "function"
				? body
				: Readable.from(Buffer.from(await new Response(body).arrayBuffer()));
	await pipeline(source, createWriteStream(local.storagePath, { flags: "wx" }));
}
